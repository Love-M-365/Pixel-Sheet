...
/>
  ...
  <div className="flex items-center justify-center ">
    ...
    <div key={10} className="h-9 dashed-vertical-border flex justify-center items-center bg-[#EEEEEE]">
  ...
  <img src={addicon} alt="add " className="w-[20px] h-[20px]"></img>
        ...
        {/* Header */}
      <div className="grid grid-cols-[2rem_256px_repeat(5,124px)_125px_125px_124px_124px_124px] sticky mt-[5px] bg-white text-gray-500 border-b border-gray-300 z-10 text-[10px]">
  <div className="h-9 flex text-[16px] text-[#AFAFAF] font-normal items-center justify-center  border-r bg-[#eeeeee]">#</div>

  {columns.map((col, index) => (
  <div
    key={col.key}
    className={clsx(
      "h-9 flex items-center justify-between px-2 font-semibold text-xs",
      index === 9 ? "dashed-vertical-border" : "border-r"
    )}
  ...
  >
    ...
    {col.hasDropdown && (
      <button className="px-1 bg-transparent rounded-md text-[#AFAFAF] flex items-center">
        ...
        {data.map((row, rowIndex) => (
          <div
            key={rowIndex}
            className="grid grid-cols-[2rem_256px_repeat(5,124px)_125px_125px_124px_124px_124px] border-b border-gray-200 bg-white hover:bg-gray-50 text-[8px]"
          >
            <div className="h-9 flex text-xs items-center justify-center  text-gray-500 border-r bg-[white]">
              {rowIndex + 1}
            </div>
            <input className="h-9 px-[6px] text-black  text-[12px] border-r outline-none bg-transparent truncate" value={row.request} onChange={(e) => handleChange(rowIndex, "request", e.target.value)} />
            <input className="h-9 px-[6px] w-full border-r text-black text-xs text-right outline-none bg-transparent truncate" value={row.submitted} onChange={(e) => handleChange(rowIndex, "submitted", e.target.value)} />
            <div className="h-9 px-[6px] border-r flex text-center justify-center items-center truncate">
              <span className={clsx("text-xs text-center font-medium px-[8px] py-1 rounded-full", statusColorMap[row.status])}>
                {row.status}
              </span>
            </div>
            <input className="h-9 text-black text-xs w-full px-2 border-r outline-none bg-transparent truncate" value={row.submitter} onChange={(e) => handleChange(rowIndex, "submitter", e.target.value)} />
            <input className="h-9 px-[6px] border-r text-xs w-full outline-none bg-transparent text-black underline truncate" value={row.url} onChange={(e) => handleChange(rowIndex, "url", e.target.value)} />
            <input className="h-9 px-[6px] border-r text-xs w-full text-black outline-none bg-transparent truncate" value={row.assigned} onChange={(e) => handleChange(rowIndex, "assigned", e.target.value)} />
            <div className="h-9 px-[6px] border-r text-xs w-full text-center justify-center flex items-center font-semibold truncate">
              <span className={clsx(priorityColorMap[row.priority])}>{row.priority}</span>
            </div>
            <input className="h-9 px-[6px] border-r text-xs w-full text-black justify-center items-center text-right outline-none bg-transparent truncate" value={row.dueDate} onChange={(e) => handleChange(rowIndex, "dueDate", e.target.value)} />
            <div className="h-9 px-[6px] border-r text-black justify-end items-center text-right flex text-xs w-full truncate">{row.estValue} <span style={{color:"grey",marginLeft:"0.5rem",textAlign:"right"}}>₹</span></div>
            <div className=" flex items-center justify-center text-gray-400 hover:text-gray-600 h-9 dashed-vertical-border cursor-pointer">
    ...
    >
      ...
      {[...Array(10)].map((_, colIndex) => (
        <div
          ...
          className={clsx(
            "flex items-center bg-transparent justify-center text-gray-400 hover:text-gray-600 h-9  cursor-pointer",
            colIndex === 9
              ? "dashed-vertical-border"
              : "border-r border-gray-200"
          )}
          
        ><input className="bg-transparent text-black  w-[124px] h-[32px] text-[12px]"/></div>
      ))}
    </div>
  );
})}
